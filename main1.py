...
def show_trainees():                                 #this part is to show trainees
    ...
    for trainee_id, trainee in trainees.items():
        ...
        print(f"Level: {trainee['level']}")
        print(f"Completed Classes: {trainee['completed_classes']}")
        print(f"Package: {trainee['package']}")
        print(f"Assigned Trainer: {trainee['assigned_trainer']}")
        print(f"Assigned Horse: {trainee['assigned_horse']}")
...
# In the horse_owner_menu()  (This part of the code was extremely difficult for me, so I had to search through several data sources to figure out how to link them.)
...
def register_trainee():
    ...
    while True:
        training_type = input("Choose training type (Jumping / Racing / For fun): ")

        if training_type.lower() == "jumping":
            training_type = "Jumping"
            break
        elif training_type.lower() == "racing":
            training_type = "Racing"
            break
        elif training_type.lower() == "for fun":
            training_type = "For fun"
            break
        else:
    ...
    while True:
        ...
        if package == 10 or package == 20 or package == 40:
            break
        else:
    ...
    while True:
        assessment = input("Enter assessment result: ")
...
def show_trainee_info(name):
    ...
    for trainee in trainees.values():
        if trainee["trainee_name"].lower() == name.lower():
            ...
            print(f"Training Type: {trainee['training_type']}")
            print(f"Level: {trainee['level']}")
            print(f"Package: {trainee['package']} Classes")
            print(f"Completed Classes: {trainee['completed_classes']}")
            print(f"Assigned Trainer: {trainee['assigned_trainer']}")
            print(f"Assigned Horse: {trainee['assigned_horse']}")
            ...
            if trainee["license"]:
                ...
                print("License Status: Not Licensed")
...
def main_menu():                    #We start the project from the main menu and a list will appear to chose from the menu
    while True:
        ...
        if choice == "1":
            ...
            print("Thank you for using the Horse Stable System.")
            break
        else:
            print("Invalid choice.")
